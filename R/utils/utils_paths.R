...
load_config <- function(config_path = NULL, root = find_repo_root()) {
  if (is.null(config_path)) {
    config_path <- Sys.getenv("ELECTROTECH_CONFIG", fs::path(root, "config", "config.yml"))
  }
...
resolve_paths <- function(cfg, root = find_repo_root()) {
  ...
  list(
    ...
    write_audit = as.logical(Sys.getenv("ELECTROTECH_WRITE_AUDIT", cfg$write_audit %||% FALSE))
  )
}
